...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .modal { background: var(--navy-mid); border: 1px solid var(--border); border-radius: 12px; padding: 40px; width: 100%; max-width: 420px; animation: slideUp 0.25s ease; }
  ...
  /* USAGE BAR */
  .usage-bar-wrap { max-width: 900px; margin: 0 auto 0; padding: 0 48px; }
  ...
  /* GATE - not logged in */
  .gate { border: 1px dashed rgba(232,162,48,0.3); border-radius: 8px; padding: 40px 32px; text-align: center; margin-top: 0; }
...
const ANALYSIS_PROMPT = `You are a legal document analyst specialising in UK law and consumer rights. Analyse the provided document and return ONLY valid JSON — no preamble, no markdown fences, no explanation.
...
Return this exact structure:
{
  ...
  "clauses": [{"name": "string", "severity": "safe"|"caution"|"danger", "plain_english": "string"}],
  "red_flags": ["string"],
  "actions": ["string"]
}

Focus on: what the person is giving up, unusual restrictions, data/privacy clauses, termination rights, IP ownership, non-compete clauses. Write for a non-lawyer UK resident. Identify 4-7 key clauses.`;
      ...
      <div className="app">
              ...
              {!authSuccess && <>
                ...
                <div className="form-group">
                  ...
                  <input className="form-input" type="password" placeholder="••••••••"
                    ...
                    onKeyDown={e => e.key === "Enter" && (authModal === "signup" ? handleSignUp() : handleSignIn())} />
                </div>
                <button className="btn-full" disabled={authLoading || !authForm.email || !authForm.password}
                  onClick={authModal === "signup" ? handleSignUp : handleSignIn}>
        ...
        {!result && !loading && (
          <>
            ...
            {/* Usage bar — only for logged-in free users */}
            {session && !isPro && (
              <div className="usage-bar-wrap">
                <div className="usage-bar">
                  ...
                  <button className="usage-upgrade" onClick={handleUpgrade} disabled={checkoutLoading}>
              ...
              <div className="upload-panel">
                ...
                {error === "free_limit" ? (
                  <div className="limit-gate">
                    ...
                    <button className="btn-upgrade" onClick={handleUpgrade} disabled={checkoutLoading}>
                      {checkoutLoading ? "Redirecting…" : "Upgrade to Pro — from £9/month →"}
                    </button>
                  </div>
                ) : !session ? (
                  /* Not logged in gate */
                  ...
                  <>
                    ...
                    {tab === "paste" && (
                      <textarea placeholder="Paste your contract, terms, or any legal text here…"
                    ...
                    <div className="analyse-row">
                      ...
                      <button className="btn-analyse" onClick={analyse} disabled={!canAnalyse || loading}>
                        Analyse document →
                      </button>
                    </div>
                  </>
